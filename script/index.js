...
function renderTask(task) {
    return `
        <div class="content__block ${task.completed ? 'completed' : ''}" data-id="${task.id}">
            <img class="content__text-icon" src="public/images/dot.png" alt="dot">
            <div class="content__text">${task.title}</div>
            <div class="content__item">
                <div class="content__priority content__priority_${task.priority}">${task.priority}</div>
                ...
                <a class="content__link" href="#" data-action="remove">
                    <img class="content__link-icon" src="public/images/garbage.svg" alt="trash" width="20">
                </a>
            </div>
        </div>
    `;
...
const todoApp = new ToDoList([
    new Task(
        'Standup meeting with the team WAYUP',
        PRIORITIES.LOW,
        '2022-02-10'
    ),
    new Task(
